...
mysql_connect('localhost','root','');
	...
	$selectNew = mysql_query("select * from delevery where o_state = 'New' and code ='$code'");
	$selectNew1 = mysql_query("select * from delevery where o_state = 'New' AND trackstate = 'Unassigened' and code ='$code'");
         ...
         $run2 = mysql_query($sql2);
                if(mysql_num_rows($run2)>0)
                {
                   ...
                   $o_no1 = $row1['o_no'];
                       ...
                       //echo "<script>alert('Data Send successfully');window.location='new_orders.php'</script>";
					   header("Refresh: 8 ; url=new_orders.php");
      
                    
                }
                else
                {
                  echo "<script>alert('request failled');window.location='new_orders.php'</script>";
                }
    ...
    <section class="sidebar">
      ...
      <!-- /.search form -->
       ...
       <ul class="sidebar-menu" data-widget="tree">
		...
		<span class="pull-right-container">
              <small class="label pull-right bg-yellow"><?php  echo mysql_num_rows($selectNew1); ?></small>
        ...
        <li class="treeview">
		 ...
		 <li class="treeview">
            <a href="#"><i class="fa fa-user"></i> <span>Users Management</span>
              <span class="pull-right-container">
                  <i class="fa fa-angle-left pull-right"></i>
                </span>
            </a>
    ...
    <div class="content-wrapper">
      ...
      <section class="content container-fluid">
		...
		$d_id = $row['d_id'];
						...
						if($state == 0){
						echo"<td><input name='deleteCb[]' type='checkbox' value=".$row['o_no']." /></td>";}else{echo "<td></td>";}
						
						echo"<td>" .$row['ref']."#".$row['d_id']."</td><td>".$row['o_location']."</td>
						<td>".$row["c_phone"]."</td><td>".$row["o_price"]."</td>
						<td>".$row["o_pic"]."</td><td>".$row["weight"]."</td><td>".$row["driver"]."</td>
						<td>".$age."</td>";
						if($state == 0){
						echo"<td><center><a  class='btn btn-app btn-sm' style ='color:blue;font:12px tohoma;' href='index-pdf-mark.php?print=".$row['o_no']."'><i class='glyphicon glyphicon-print'></i> </a></center></td>
                    ...
                    //$conn -> close();
echo "</table><select  class='btn btn-primary bg-green' name='ind'><option value=''> ---Select Driver--- </option>";
echo $dd_res=mysql_query("SELECT u_name, u_id from driver where u_val = '$code'");
        ...
        <?php }
if($state == 0){echo"</select>&nbsp;&nbsp;  &nbsp;&nbsp<input name='delete' value='Send Selected' type='submit'  class='btn btn-primary bg-sm'/>
					  &nbsp; &nbsp &nbsp; &nbsp &nbsp; &nbsp &nbsp; &nbsp <input name='print' value='Print PDF' type='submit'  class='btn btn-primary bg-sm' formaction ='printdriver.php'/>
					  &nbsp; &nbsp &nbsp; &nbsp &nbsp; &nbsp &nbsp; &nbsp 
					<input name='print2' value='Export Select orders to Excell' type='submit'  class='btn btn-primary bg-red' formaction ='sel_excell.php'/>"; 
}
					?>
