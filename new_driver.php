...
$selectAll = mysql_query("select * from delevery  where trackstate != 'Unassigened' ORDER BY o_no ASC");
	...
	$selectdriver = mysql_query("select * from driver where u_val = '$get'");

	
	$lim = mysql_num_rows($selectdriver);
                     ...
                     <form class="form-horizontal" action="#" method="post">
                      <div class="tile-body">
                          ...
                          <div class="form-group row">
                            ...
                            <div class="col-md-8">
                             <?php if($lim < $orders){ echo'<input type="submit" name="registerdo" class=" btn btn-primary" value="Add Driver">';}
							 
							 else{echo "<p class='btn btn-primary bg-red'>Your Limit is not engouh to Add New Driver</p>";}?>
                            </div>
                          </div>
                  <!-- /.box-footer-->
                </div>
...
if(isset($_POST['registerdo']))
{
  ...
  {
      $sql = mysql_query("SELECT u_email FROM driver WHERE u_email = '$u_email'");
      ...
      {  
         ...
         $sql1 = mysql_query( "INSERT INTO driver(u_name,  u_pass, u_email, phone, u_val) VALUES ('$u_name1',  '$u_pass', '$u_email', '$phone', '$ucode')");
        
          if($sql1)
          {
             echo "<script>alert('Register successfully');window.location='all_drivers.php';</script>";
          }
          else
          {
