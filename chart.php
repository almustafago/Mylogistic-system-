...
$selectAll = mysql_query("select * from delevery  where trackstate != 'Unassigened'");
	$selectNew = mysql_query("select * from delevery where o_state = 'New' AND trackstate = 'Unassigened'");
...
<?php  
 ...
 $query = "SELECT o_state, count(*) as number FROM delevery   GROUP BY o_state";  
 $result = mysqli_query($connect, $query); 


$query1 = "SELECT driver, count(*) as number1 FROM delevery  GROUP BY driver";  
...
<option value='Failed'> Failed </option>
                  ...
                  <div class="box-body table-responsive">
                    ...
                    $selectAl = mysql_query("select * from delevery  order by send_date  DESC  LIMIT 30");
		...
		$d_id = $row['d_id'];
						...
						echo"<td>" .$row['ref']."#".$row['d_id']."</td><td>".$row['o_location'].
                          "</td><td>".$row["c_phone"]."</td><td>".$row["o_price"]."</td><td>".$row["o_state"]."</td><td>".$row["o_state"]."</td><td>".$row["driver"]."</td><td>".$row["send_date"]."</td></tr>";
                      }
                    }
                    //$conn -> close();
              ...
              <div class="box-body table-responsive">
                <table id="example2" class="table table-bordered table-striped">
                 <thead>
                    ...
                    $selectAl = mysql_query("select driver, count(1) as number from delevery  where trackstate = 'Completed' AND driver !=' ' group by driver  order by number  DESC ");
$i =1;
                    if (mysql_num_rows($selectAl)> 0) {
                      while ($row = mysql_fetch_assoc($selectAl)) {
